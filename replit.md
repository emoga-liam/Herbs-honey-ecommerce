...
- See `pnpm-workspace.yaml` for workspace package paths and `onlyBuiltDependencies` configuration.
- See `.npmrc` for `node-linker=hoisted` and `only-built-dependencies` settings (Hostinger compatibility).
